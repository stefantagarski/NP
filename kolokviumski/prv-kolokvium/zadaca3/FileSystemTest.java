...
interface IFile extends Comparable<IFile> {
    ...
    long getFileSize(); //?? so rekuzija sum na goleminite

    String getFileInfo(String indent); //?? identification ,rekurzija

    void sortBySize(); //rekurzija

    long findLargestFile(); //rekurzija
...
class Folder extends File implements IFile {
    ...
    public String getFileInfo(String indent) {
       ...
       sb.append(files.stream().map(files -> files.getFileInfo(indent+"    "))
